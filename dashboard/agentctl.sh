...
WORKTREE_ROOT_SETTING="${AGENTSTACK_WORKTREE_ROOT:-$HERE/../worktrees}"
...
render_plist() {
  ...
  sed \
    ...
    -e "s|__CLAUDE_CONFIG_DIR__|$(xml_sed_escape "$CLAUDE_CONFIG_DIR_SETTING")|g" \
  ...
  if [[ -z "$CLAUDE_CONFIG_DIR_SETTING" ]]; then
    sed '/<key>CLAUDE_CONFIG_DIR<\/key>/{N;d;}' "$PLIST_DST" > "$PLIST_DST.tmp"
...
export_background_env() {
  ...
  export AGENTSTACK_WORKTREE_ROOT="$WORKTREE_ROOT_SETTING"
  ...
  if [[ -n "$CLAUDE_CONFIG_DIR_SETTING" ]]; then
    export CLAUDE_CONFIG_DIR="$CLAUDE_CONFIG_DIR_SETTING"
  else
