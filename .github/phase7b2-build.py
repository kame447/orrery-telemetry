...
REFERENCE = Path('.phase7b2-reference-server.py')
SERVER = Path('dashboard/server.py')
LIVE_TEST = Path('tests/test_dashboard_live_read_scope.py')
CODEX_TEST = Path('tests/test_dashboard_codex_liveness.py')
TS_TEST = Path('tests/test_graph_timestamp_compat.py')
...
# Phase 7b2 only wires the already-reviewed attribution primitive into live
# read surfaces. Keep mutation/control helpers untouched.
for name in ('tmux_state', 'agentmail_state', 'build_agents', 'graph_payload'):
    replacement = function_source(reference, name)
    if name in {'agentmail_state', 'build_agents'}:
        replacement = replacement.replace(
            '_project_key()', '_canonical_dashboard_project_key()'
        )
    current = replace_function(current, name, replacement)

cache_old = '_GRAPH_CACHE: dict = {"ts": 0, "data": None}'
cache_new = '''_GRAPH_CACHE: dict = {
    "ts": 0, "project_key": None, "live_parents": None, "data": None,
}'''
if current.count(cache_old) != 1:
    raise RuntimeError('graph cache anchor drifted')
current = current.replace(cache_old, cache_new, 1)

scoped_lineage = function_source(reference, '_scoped_live_parents')
if 'def _scoped_live_parents(' not in current:
    current = insert_before_function(current, '_raw_graph', scoped_lineage)

raw_graph = function_source(reference, '_raw_graph').replace(
    '_project_key()', '_canonical_dashboard_project_key()'
)
current = replace_function(current, '_raw_graph', raw_graph)

# The reference graph_payload already uses gated sessions + scoped lineage.
ast.parse(current)
SERVER.write_text(current, encoding='utf-8')

# Existing tests that deliberately stub project attribution remain explicit
# about doing so; this keeps Phase 7b2 from turning unrelated liveness tests
# into host-environment integration tests.
codex = CODEX_TEST.read_text(encoding='utf-8')
anchor = '    monkeypatch.setattr(server, "_project_key", lambda: "")\n    return root\n'
replacement = '''    monkeypatch.setattr(server, "_project_key", lambda: "")
    ...
    return root
'''
if codex.count(anchor) != 1:
    raise RuntimeError('codex liveness fixture anchor drifted')
codex = codex.replace(anchor, replacement, 1)
codex, count = re.subn(
    r'("_raw_graph",\n\s*)lambda: \{',
    r'\1lambda _live_parents=None: {',
    codex,
)
if count != 1:
    raise RuntimeError(f'expected one codex _raw_graph lambda, found {count}')
CODEX_TEST.write_text(codex, encoding='utf-8')

ts = TS_TEST.read_text(encoding='utf-8')
old = 'monkeypatch.setattr(server, "_raw_graph", lambda: unhealthy)'
new = 'monkeypatch.setattr(server, "_raw_graph", lambda _live_parents=None: unhealthy)'
if ts.count(old) != 1:
    raise RuntimeError('timestamp _raw_graph fixture anchor drifted')
TS_TEST.write_text(ts.replace(old, new, 1), encoding='utf-8')

LIVE_TEST.write_text(r'''"""Project-scoped live Dashboard read regression tests."""
from __future__ import annotations
