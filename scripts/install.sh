...
if [[ -z "${CLAUDE_CONFIG_DIR+x}" ]]; then
  CLAUDE_CONFIG_DIR_SETTING="$(agentstack_installed_env_value CLAUDE_CONFIG_DIR "$INSTALL_DIR/env.sh")"
fi
...
export AGENTSTACK_INSTALL_CLAUDE_CONFIG_DIR="$CLAUDE_CONFIG_DIR_SETTING"
...
values = {
    ...
    "CLAUDE_CONFIG_DIR": os.environ.get("AGENTSTACK_INSTALL_CLAUDE_CONFIG_DIR", ""),
...
for key, value in values.items():
    if key == "CLAUDE_CONFIG_DIR" and not value:
...
repl = {
    ...
    "__CLAUDE_CONFIG_DIR__": xml.sax.saxutils.escape(os.environ.get("AGENTSTACK_INSTALL_CLAUDE_CONFIG_DIR", "")),
...
if not os.environ.get("AGENTSTACK_INSTALL_CLAUDE_CONFIG_DIR", ""):
    text = text.replace("    <key>CLAUDE_CONFIG_DIR</key>\n    <string></string>\n", "")
...
for key, value in env.items():
    if key == "CLAUDE_CONFIG_DIR" and not value:
        lines.append("UnsetEnvironment=CLAUDE_CONFIG_DIR")
    else:
