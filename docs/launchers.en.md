...
Every child launch path (direct, preregistered Claude/Codex, and Gemini) first validates the child's actual work directory against the project key with the same check registration uses. A Git repository and all its linked worktrees share one namespace. A logical key needs the launcher's matching repository. The validated canonical key is used for Mail, and the child session receives the full project/repository/work-directory/protected-root tuple. `agentstack-preregister-child` accepts `--work-dir <dir>` (default: the current directory) and validates it before any Mail request. It refuses to overwrite a runtime token or child state that already exists under the same name. Its `.binding.json` receipt (now written for every program) records the name, project, repository and work directory. Before a preregistered launch copies anything, ORRERY Mail must accept the handoff token for that name in that project (`whois` with the token), any receipt must match, and any name-keyed local credential must hold the same token. A refusal leaves the one-shot handoff and existing files untouched. The handoff is consumed only after the child has started. A failed preregistered launch does not retire anything: it removes only the runtime copies it installed (those still holding its token) and keeps the private handoff. A failed direct launch removes its runtime credentials only after a confirmed retirement. The direct path first saves the token the server returned. If the returned name already has local credentials, or the token cannot be installed, it retires that registration with the saved token and keeps the private one-shot file (reporting its path) unless the retirement is confirmed. The Gemini child launcher, its runner and the preregistered Gemini adapter pass the one-shot file's own token to the common cleanup (read privately at run time). They drop the file only after that cleanup succeeded, which requires Mail's retirement receipt even when the durable copy has disappeared, or, for the child launcher and its runner, while a private durable copy of the same token exists. `agentstack-preregister-child` fails explicitly, keeping the handoff, when it cannot store the durable token.

**Child cleanup ownership.** `cleanup-child-agent.sh` acts only on a child named by its argument or entry `AGENT_NAME`, never on one inferred from the pane or session. The private token file, child state, and any `CHILD_REGISTRATION_TOKEN` must agree. The recorded project (or, without state, the live key) must validate for the directory cleanup runs in, and its recorded repository or work directory must match. ORRERY Mail must then accept the credential for that name and project. Only then does it attempt to release reservations (best effort, as before). It re-reads the credentials before retiring. Local files are deleted only after Mail returns its retirement receipt for that agent and project (`status: retired`), and after one more re-read. A transport, HTTP, JSON-RPC or tool error, or an empty or mismatched answer, keeps them and cleanup exits non-zero. If anything changed, the newer files are kept too. An ambient project key from another repository, a token Mail rejects (including one already retired), or a foreign directory never authorizes cleanup; the files stay for inspection.
