...
Antigravity CLI の read-only print command を利用する。

```sh
agy -p "/usage" --output-format json
```

`command.data.groups[].buckets[]` に実際に返された quota bucket のみを正規化する。Gemini / Claude / GPT などの固定 bucket を ORRERY 側では作らない。
