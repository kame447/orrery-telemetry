...
- **Grace / debounce:** `release-file-reservation.sh` waits for `AGENTSTACK_RELEASE_GRACE_SECONDS` (90 seconds by default; legacy `FILE_RESERVATION_RELEASE_GRACE_SECONDS` is also accepted) before [`release-file-reservation-worker.py`](../hooks/release-file-reservation-worker.py) releases the reservation. State is under `$AGENTSTACK_RUNTIME_DIR/file_release_debounce/`; the next edit updates the token, and a new-reservation hook removes the state file so the older worker becomes a no-op. A state slot is named by project namespace, agent, and relative path, so the same agent name and path in two projects never cancel each other. Slots armed by the previous version (agent and path only) cannot be attributed to a project, so the invalidation hook leaves them alone. When such a worker wakes, the shared release function refuses its request because its slot is not named for a project (`error=unscoped-release-worker` in `release-failures.log`). The worker then removes only its own slot. The same boundary refuses a worker whose slot path is not exactly the project-scoped slot under this runtime directory, a worker carrying only one of its two markers, and a worker whose slot no longer holds its token. After an upgrade, a reservation that was pending release under the old version therefore expires by its TTL instead of being released early. This protection reaches sleeping workers only because they re-source the installed `reservation-common.sh` when they wake. Workers started from an immutable older checkout load that checkout's library, so let them drain (wait out the grace period, 90 seconds by default) before switching over.
...
Every signal carries the canonical `project_key` of the project it was sent in. A signal that predates that field is delivered only when Mail's own project resource names the project for its slug (the answer must be exactly one entry whose slug matches and whose `human_key` is non-empty); the slug alone is never proof, and an unavailable, ambiguous or malformed answer leaves the signal pending. That lookup is bounded on purpose: the bundled server publishes no MCP resources, so against it such a signal simply stays pending, and the path only completes where a server provides that resource. The current producer always writes the canonical key, so this concerns signals written before the upgrade.

The delivery target is the tmux session whose name exactly matches the agent (`=name`), and then one concrete pane: every later call targets that pane id. Before the pane is read or written, the recipient is proven: its own directory and the project variables of its own session must resolve, through the same validator the launchers use, to the signal's project, and the agent's private `agent_token_<name>` (mode 0600, no symlink) must be accepted by Mail's `whois` for that agent in that project. Those variables are read as one complete `show-environment` snapshot of the session whose id that pane reported, never by the agent's name, which can move to another session at any moment; only the six context variables are read, literally, and nothing is sourced or evaluated. A failed, timed-out or killed read is a failure of the whole proof and never a session that carries no markers, and the pane's id, session, name and directory are read again after the snapshot and after the Mail proof, together with the markers themselves, and must all be unchanged: a session that re-points only its project variables while Mail answers keeps the same pane, id, name and directory. Every session marker that is present has to agree — a project key cannot mask a second key, a repository, a work directory, a worktree root or a protected root belonging elsewhere — while any worktree of the same repository still counts. A session with no such variables can only prove its directory's own project. The watcher's own project selection is never used as recipient evidence, and a recipient that cannot be proven leaves the signal pending without any `capture-pane` or `send-keys`.

Notification text is sent literally and submitted with a separate `C-m` call. The full evidence, including a current `whois` proof, is re-established immediately before each of those two writes: a pane, session, directory, any context marker (including the worktree root and the protected roots) or token that changed, or a credential Mail no longer accepts — the registration deleted or the name re-registered with another token, which a soft retirement is not — suppresses the remaining write and leaves the signal pending. This narrows the delay between the two writes; it does not make them atomic. tmux calls run in timeout-controlled workers so a server stall cannot stop the entire watcher.
