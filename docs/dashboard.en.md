...
| Resume a finished agent | For tmux-based Claude / Codex CLI agents, show past agents with DECK history `30d` / `all` or NETWORK `ALL`. Restore from a card marked `RESUME READY`, or from `RESUME · VERIFY` on an older Claude row. For the latter, one `VERIFY & RESUME` click in the detail panel performs verification and continues through resume. NETWORK `Select` counts only pre-verified gone / retired nodes whose `resume_capability` is `ready` in `Resume N`. Resume requires a verified transcript, original cwd, corresponding CLI, and terminal adapter; a Codex child also requires provenance, formal registration, a credential, and restorable configuration. |
