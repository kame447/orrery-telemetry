...
すべての child 起動経路（direct、pre-registered の Claude/Codex、Gemini）は、まず child の実際の work directory を registration と同じ検証で project key と照合します。Git repository とその linked worktree は1つの namespace を共有し、logical key は launcher が渡す一致した repository がある場合だけ有効です。Mail には検証済みの canonical key を使い、child session には project・repository・work directory・protected root の組を渡します。`agentstack-preregister-child` は `--work-dir <dir>`（既定は現在の directory）を受け付け、Mail への request より前に検証します。同じ名前の runtime token や child state が既にあれば上書きを拒否します。`.binding.json` receipt は全 program で作られ、name・project・repository・work directory を記録します。pre-registered launch は何かを複製する前に、その名前・project で ORRERY Mail が handoff token を受け付けること（token 付き `whois`）、receipt が一致すること、同名の local credential が同じ token を持つことを確認します。拒否時は one-shot handoff も既存 file も変更しません。handoff は child の起動後にだけ消費します。pre-registered launch の失敗時は何も retire せず、自分が配置した runtime copy（まだその token を持つもの）だけを消して private な handoff を残します。direct launch の失敗時は、retirement が確認できた後にだけ runtime credential を消します。direct 経路はまず server が返した token を保存します。返された名前に既存の local credential がある場合や token を配置できない場合は、その token で登録を retire し、確認できなければ private な one-shot file を残してその path を表示します。Gemini child launcher、その runner、pre-registered Gemini adapter は、one-shot file 自身の token を実行時に private に読み、共通 cleanup に渡します。one-shot file を消すのはその cleanup が成功した後（durable copy が消えていても Mail の retirement receipt が必要）か、child launcher と runner では同じ token の private な durable copy がある間だけです。`agentstack-preregister-child` は durable token を保存できなければ handoff を残したまま明示的に失敗します。

**child cleanup の所有確認。** `cleanup-child-agent.sh` は引数または起動時の `AGENT_NAME` で明示された child だけを対象にし、pane や session から推測しません。private な token file、child state、`CHILD_REGISTRATION_TOKEN` のうち存在するものは一致している必要があります。記録された project（state が無ければ live key）が cleanup を実行する directory に対して検証でき、記録された repository または work directory も一致しなければなりません。そのうえで ORRERY Mail がその名前・project でその credential を受け付けたときだけ reservation の解放を試みます（従来どおり best effort）。retire 前に credential を読み直します。local file は、Mail がその agent・project の retirement receipt（`status: retired`）を返し、さらに読み直した後でだけ削除します。transport・HTTP・JSON-RPC・tool error や空・不一致の応答では file を残して非ゼロで終了し、途中で変化した場合も新しい file を残します。別 repository の ambient project key、Mail が拒否する token（retire 済みを含む）、無関係な directory は cleanup の根拠にならず、file は調査用に残ります。
