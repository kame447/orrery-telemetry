...
- **grace / debounce:** `release-file-reservation.sh` は `AGENTSTACK_RELEASE_GRACE_SECONDS`（既定90秒、旧 `FILE_RESERVATION_RELEASE_GRACE_SECONDS` も可）だけ待ってから [`release-file-reservation-worker.py`](../hooks/release-file-reservation-worker.py) が解放します。state は `$AGENTSTACK_RUNTIME_DIR/file_release_debounce/` にあり、次の edit は token を更新し、再予約 hook は state file を消して古い worker を no-op にします。state slot は project namespace・agent・相対 path で名付けるため、2つの project の同じ agent 名・path が互いの release を取り消すことはありません。旧版が作った agent・path だけの slot は project に帰属できないため、再予約 hook は消しません。その worker が起きたとき、共通の release 関数は project で名付けられていない slot からの要求として拒否し（`release-failures.log` に `error=unscoped-release-worker`）、worker は自分の slot だけを片付けます。同じ境界は、slot path がこの runtime directory 配下の project 付き slot と完全一致しない worker、2つの marker の片方しか持たない worker、slot が自分の token を保持していない worker も拒否します。そのため upgrade 前に release 待ちだった reservation は早期に解放されず、TTL で失効します。この保護が眠っている worker に届くのは、起床時に配置済みの `reservation-common.sh` を読み直すからです。変更されない古い checkout から起動した worker はその checkout の library を読むため、切り替え前に grace 期間（既定90秒）を待って流し切ってください。
...
signal には送信元 project の canonical な `project_key` が入ります。この field を持たない旧 signal は、Mail の project resource がその slug の project を答えたときだけ配送します（応答は slug が一致し `human_key` が空でない1件だけ）。slug だけでは証拠にならず、応答が無い・曖昧・壊れている場合は pending のままです。この経路は意図的に限定的です。同梱 server は MCP resource を一切公開しないため、同梱 server に対してはこうした signal は pending のままとなり、その resource を提供する server でのみ成立します。現在の producer は常に canonical な key を書くので、対象は upgrade 前に書かれた signal だけです。

配送先は agent 名と完全一致する tmux session（`=name`）で、そこから具体的な pane を1つ決め、以降の call はその pane id を対象にします。pane を読む・書く前に受信者を証明します。その pane 自身の directory と、その session 自身の project 変数が、launcher と同じ validator で signal の project に解決でき、さらに agent の private な `agent_token_<name>`（mode 0600、symlink 不可）を Mail の `whois` がその project・agent で受け付ける必要があります。project 変数は、その pane 自身が報告した session id を対象にした1回の完全な `show-environment` snapshot として読みます。agent 名で読むと、名前が別 session へ移った瞬間にその session が答えてしまうためです。読むのは既定の6変数だけで、literal に解釈し、source も eval もしません。読み取りの失敗・timeout・強制終了は「marker が無い session」ではなく証明全体の失敗として扱い、snapshot の後と Mail 証明の後に、pane id・session・名前・directory と marker 自体を読み直し、いずれも変化していないことを要求します。Mail の応答を待つ間に project 変数だけを別 project へ差し替えた session は、pane・id・名前・directory が同じままだからです。session が持っている marker はすべて一致していなければなりません。project key が2つ目の key や、別 project の repository・work directory・worktree root・protected root を覆い隠すことはできません（同じ repository の worktree は同一とみなします）。これらの変数が1つも無い session は、自分の directory 自身の project しか証明できません。watcher 自身の project 選択は受信者の証拠にせず、証明できない受信者には `capture-pane` も `send-keys` も行わず signal を pending のまま残します。

通知 text は literal send し、submit は別 call の `C-m` で送ります。この2回の書き込みの直前には毎回、現在の `whois` 証明を含む証拠一式を取り直します。pane・session・directory・context marker（worktree root と protected roots を含む）・token が変わった場合や、Mail が credential を受け付けなくなった場合（登録の削除、別 token での再登録。soft retirement はこれに当たりません）は残りの書き込みを行わず pending にします。これは2回の書き込みの間の隙間を狭めるもので、atomic にするものではありません。tmux call は timeout 付き worker に分離し、server stall が watcher 全体を止めないようにします。
