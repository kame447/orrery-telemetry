...
def test_running_provider_card_keeps_dashboard_exit_control():
    source = (ROOT / "dashboard" / "index.html").read_bytes()
    rendered = server._render_dashboard_index(source).decode("utf-8")
    assert "const exitBtn=a.running" in rendered
    assert 'class="exitbtn"' in rendered
    assert "exitAgent(event" in rendered
    assert "/api/exit" in rendered
